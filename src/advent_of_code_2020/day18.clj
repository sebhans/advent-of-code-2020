...
(defmacro the-new-math
  "Evaluates infix addition and multiplication left to right,
  honouring parentheses."
  ([expr]
   (if (seq? expr)
     `(the-new-math ~@expr)
     expr))
  ([left op right]
   `(~op (the-new-math ~left) (the-new-math ~right)))
  ([left op right & tail]
   `(the-new-math (~left ~op ~right) ~@tail)))

(defn sum-with-math-rules
  "Evaluates a series of expressions, one per line, according to the math rules
  with the given name and returns the sum of the resulting values."
  [rule-name s]
  (->> s
       s/split-lines
...
(def solve-1
  "Evaluates a series of expressions, one per line, according to the new math
  and returns the sum of the resulting values."
  (partial sum-with-math-rules "the-new-math"))

(defmacro advanced-math
  "Evaluates infix addition and multiplication with reversed operator precedence,
  honouring parentheses."
  ([expr]
   (if (seq? expr)
     `(advanced-math ~@expr)
     expr))
  ([left op right]
   `(~op (advanced-math ~left) (advanced-math ~right)))
  ([left op right & tail]
   (if (= op '+)
     `(advanced-math (~left ~op ~right) ~@tail)
     `(advanced-math ~left ~op (~right ~@tail)))))

(def solve-2
  "Evaluates a series of expressions, one per line, according to advanced math
  and returns the sum of the resulting values."
  (partial sum-with-math-rules "advanced-math"))

(def trial-input "1 + 2 * 3 + 4 * 5 + 6
1 + (2 * 3) + (4 * (5 + 6))
2 * 3 + (4 * 5)
5 + (8 * 3 + 9 + 3 * 4 * 3)
5 * 9 * (7 * 3 * 3 + 9 * 3 + (8 + 6 * 4))
...
(def real-input "9 + 3 * (9 * (9 + 3 + 5 * 3) * 5 * 3 * 5)
