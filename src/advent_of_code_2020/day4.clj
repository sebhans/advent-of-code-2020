...
(def field-validators
  ...
  {"byr" #(in-range % 1920 2002)
   "iyr" #(in-range % 2010 2020)
   "eyr" #(in-range % 2020 2030)
   "hgt" #(when-let [match (re-matches #"(\d+)(cm|in)" %)]
            (case (nth match 2)
              "cm" (in-range (nth match 1) 150 193)
              "in" (in-range (nth match 1) 59 76)))
...
(def trial-input "ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
(def trial-input-2 "eyr:1972 cid:100
...
(def real-input "hcl:#6b5442 ecl:brn iyr:2019
